...
第一次的时候，数组长度为默认值16，阈值为16*0.75=12，走的`代码4`逻辑，等到数组长度超过阈值12后，触发第二次扩容，此时table数组，和threshold都不为0，即oldTab、oldCap、oldThr都不为0，先走`代码1`，如果oldCap长度的2倍没有超过最大容量，及oldCap 长度大于等于 默认容量16，那么下次扩容的阈值 变为oldThr大小的两倍即 12 * 2 = 24，下面继续走`代码5`计算新阈值为 24 * 0.75 = 18
